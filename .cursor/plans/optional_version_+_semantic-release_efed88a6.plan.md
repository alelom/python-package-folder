---
name: Optional version + semantic-release
overview: Make --version optional for both (1) subfolder builds and (2) main-package builds with shared code by resolving the next version via Node.js semantic-release. Subfolder uses per-package tags and path-filtered commits; main package uses repo-level tags. README is updated to document and illustrate both workflows.
todos: []
isProject: false
---
